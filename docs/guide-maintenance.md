...
2. Let GitHub Actions run `Build guide HTML`.
...
The workflow is `.github/workflows/build-guide.yml`.
...
If the workflow does not run automatically, ask the user to run it manually:

Actions → Build guide HTML → Run workflow → branch `main`.

If the workflow completes but no new generated commit appears, that usually means the rendered HTML did not change.
...
5. If setup or permissions fail, patch `.github/workflows/build-guide.yml`.
...
2. ChatGPT updates Markdown, primer, or small tool/workflow files directly on `main`.
3. GitHub Actions regenerates `llm_coding_workflow_guide.html`.
4. GitHub Actions validates the generated guide.
5. GitHub commits the regenerated HTML if it changed.
6. ChatGPT reports source commit, generated HTML status, validation status, and follow-up steps.
